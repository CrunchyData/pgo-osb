...
func runWithContext(ctx context.Context) error {
	...
	if options.Insecure {
		...
		if options.TLSCert != "" && options.TLSKey != "" {
			fmt.Println("Starting secure broker with TLS cert and key data")
			err = s.RunTLS(ctx, addr, options.TLSCert, options.TLSKey)
		} else {
			if options.TLSCertFile == "" || options.TLSKeyFile == "" {
				fmt.Println("unable to run securely without TLS Certificate and Key. Please review options and if running with TLS, specify --tls-cert-file and --tls-private-key-file or --tlsCert and --tlsKey.")
				return nil
			}
			fmt.Println("Starting secure broker with file based TLS cert and key")
